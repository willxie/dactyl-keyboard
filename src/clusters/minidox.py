...
class MinidoxCluster(ca.ClusterBase):
    ...
    def thumb(self, side="right"):
        print('thumb()')
        shape = self.thumb_fx_layout(self.g.rotate(self.sh.single_plate(side=side), [0.0, 0.0, -90]))
    ...
    def screw_positions(self):
        ...
        position[1] = position[1] - .4 * (self.tp.minidox_Usize - 1) * sa_length
        position[2] = 0

        return position

    def thumb_pcb_plate_cutouts(self, side="right"):
        shape = self.thumb_fx_layout(self.sh.plate_pcb_cutout(side=side))
