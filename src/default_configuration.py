...
class OLEDUndercut(OLEDConfiguration):
    oled_configuration_name = 'UNDERCUT'
    # Common parameters
...
class OLEDSliding(OLEDConfiguration):
    oled_configuration_name = 'SLIDING'
    # Common parameters
...
class ShapeConfiguration:
    ...
    tbiw_top_wall_translational_offset = (0.0, 0.0, 0.0)
    tbiw_top_wall_rotation_offset = (0.0, 0.0, 0.0)
    ...
    0, 0, 2.0)  # applied to the socket and sensor, large values will cause web/wall issues.
    ...
    oled_config = OLED_LOOKUP[oled_mount_type]()
